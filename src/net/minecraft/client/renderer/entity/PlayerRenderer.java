...
// Added null check for NoRender function
            ...
            try {
                ...
                if (Polaroid.getInstance().getFunctionRegistry().getNoRender() != null

                        && Polaroid.getInstance().getFunctionRegistry().getNoRender().element != null) {

                    hidePlayers = Polaroid.getInstance().getFunctionRegistry().getNoRender().element.getValueByName("Игроки") != null

                            && Polaroid.getInstance().getFunctionRegistry().getNoRender().element.getValueByName("Игроки").get();
            ...
            if (hidePlayers) {
                ...
                boolean customModelsEnabled = Polaroid.getInstance().getFunctionRegistry().getCustomModels().isState();

                boolean applyToFriends = Polaroid.getInstance().getFunctionRegistry().getCustomModels().friends.get();
    ...
    public ResourceLocation getEntityTexture(AbstractClientPlayerEntity entity) {
        ...
        boolean shouldUseCustomModel = Polaroid.getInstance().getFunctionRegistry().getCustomModels().isState() &&

                (isLocalPlayer || (Polaroid.getInstance().getFunctionRegistry().getCustomModels().friends.get() && isFriend));



        if (shouldUseCustomModel) {

            if (Polaroid.getInstance().getFunctionRegistry().getCustomModels().models.is("Crazy Rabbit")) {

                return new ResourceLocation("polaroid/images/modules/customModels/rabbit.png");

            } else if (Polaroid.getInstance().getFunctionRegistry().getCustomModels().models.is("Red Demon")) {

                return new ResourceLocation("polaroid/images/modules/customModels/reddemon.png");

            } else if (Polaroid.getInstance().getFunctionRegistry().getCustomModels().models.is("Freddy Bear")) {

                return new ResourceLocation("polaroid/images/modules/customModels/freddy.png");

            } else if (Polaroid.getInstance().getFunctionRegistry().getCustomModels().models.is("White Demon")) {

                return new ResourceLocation("polaroid/images/modules/customModels/whitedemon.png");
...

    {
        ...
        // CustomWorld small player integration
        if (polaroid.client.modules.impl.render.CustomWorld.child && entitylivingbaseIn == net.minecraft.client.Minecraft.getInstance().player) {
...

    {
        ...
        // Проверяем, включен ли модуль ScoreBoardHP
        ScoreboardHealth scoreboardHealthModule = Polaroid.getInstance().getFunctionRegistry().getScoreboardHealth();
        boolean useRealHp = scoreboardHealthModule != null && scoreboardHealthModule.isState();
...

        {
...

            {
                ...
                if (useRealHp) {
                    // Используем RealHpUtility для получения правильного HP (игнорируем скорборд)
                    float realHp = RealHpUtility.getRealHp(entityIn);
                    
                    // Определяем цвет в зависимости от HP
                    String color;
                    if (realHp > 15) {
                        color = "§a"; // Зеленый - много HP
                    } else if (realHp > 10) {
                        color = "§e"; // Желтый - среднее HP
                    } else if (realHp > 5) {
                        color = "§6"; // Оранжевый - мало HP
                    } else {
                        color = "§c"; // Красный - очень мало HP
                    }
                    
                    // Отображаем реальное HP с цветом
                    super.renderName(entityIn, new StringTextComponent(color + String.format("%.1f", realHp) + " ❤"), matrixStackIn, bufferIn, packedLightIn);
                } else {
                    // Модуль выключен - используем стандартную логику
                ...
                // Если скорборд не показывает HP, но модуль включен - показываем реальное HP игрока
                float realHp = RealHpUtility.getRealHp(entityIn);
