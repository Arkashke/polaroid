...
import polaroid.client.modules.impl.player.NoEntityTrace;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    private void hurtCameraEffect(MatrixStack matrixStackIn, float partialTicks) {
        ...
        Polaroid.getInstance().getEventBus().post(eventCancelOverlay);
                ...
                if (this.mc.gameSettings.getPointOfView().func_243192_a() && !flag && !this.mc.gameSettings.hideGUI
                    ...
                    if (Config.isShaders()) {
                        ...
                        if (glassHand.isState()) {
                            ...
                            // HandChams integration
                            HandChams handChams = functionRegistry.getHandChams();
                            if (handChams != null && handChams.isState()) {
                                handChams.getHandsFramebuffer().setup();

                                this.itemRenderer.renderItemInFirstPerson(p_renderHand_3_, p_renderHand_1_,
                                        this.renderTypeBuffers.getBufferSource(), this.mc.player,
                                        this.mc.getRenderManager().getPackedLight(this.mc.player, p_renderHand_3_));
                                handChams.getHandsFramebuffer().unbindFramebuffer();
    ...
    public void resetProjectionMatrix(Matrix4f matrixIn) {
        ...
        GlStateManager.multMatrix(matrixIn);
    ...
    public Matrix4f getProjectionMatrix(ActiveRenderInfo activeRenderInfoIn, float partialTicks,
        ...
        float aspectRatio = (float) this.mc.getMainWindow().getFramebufferWidth()
                / (float) this.mc.getMainWindow().getFramebufferHeight();
        
        // AspectRatio module integration
        polaroid.client.modules.impl.render.AspectRatio aspectRatioModule = 
            polaroid.client.Polaroid.getInstance().getFunctionRegistry().getAspectRatio();
        if (aspectRatioModule != null && aspectRatioModule.isState()) {
            aspectRatio *= aspectRatioModule.width.get().floatValue();
        }
    ...
    private void func_243497_c(float p_243497_1_) {
        ...
        RenderSystem.blendFuncSeparate(1, 1, 1, 1);
