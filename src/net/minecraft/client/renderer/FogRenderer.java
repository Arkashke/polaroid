...
public class FogRenderer
{
    ...
    public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier)
    {
        ...
        else
        {
            ...
            // Check if CustomWorld fog is enabled - if so, skip weather modifications
            polaroid.client.modules.impl.render.CustomWorld customWorldCheck = 
                polaroid.client.Polaroid.getInstance().getFunctionRegistry().getCustomWorld();
            boolean customFogEnabled = customWorldCheck != null && customWorldCheck.isState() && customWorldCheck.isEnabled("Кастомный туман");
        ...
        // CustomWorld custom fog integration
        polaroid.client.modules.impl.render.CustomWorld customWorld = 
            polaroid.client.Polaroid.getInstance().getFunctionRegistry().getCustomWorld();
        if (customWorld != null && customWorld.isState() && customWorld.isEnabled("Кастомный туман")) {
            if (polaroid.client.modules.impl.render.CustomWorld.mode.is("Клиент")) {
                // Client preset fog color - more natural atmospheric fog
                // Slightly desaturated blue-gray for realistic fog effect
                red = 0.75F;
                green = 0.8F;
                blue = 0.85F;
            } else if (polaroid.client.modules.impl.render.CustomWorld.mode.is("Свой")) {
                int fogColor = polaroid.client.modules.impl.render.CustomWorld.colorFog.get();
                float targetRed = ((fogColor >> 16) & 0xFF) / 255.0F;
                float targetGreen = ((fogColor >> 8) & 0xFF) / 255.0F;
                float targetBlue = (fogColor & 0xFF) / 255.0F;
                
                // Smooth transition to custom fog color for more natural effect
                float blendFactor = 0.7F; // 70% custom color, 30% original for natural blending
                red = red * (1.0F - blendFactor) + targetRed * blendFactor;
                green = green * (1.0F - blendFactor) + targetGreen * blendFactor;
                blue = blue * (1.0F - blendFactor) + targetBlue * blendFactor;
            }
        }
    ...
    public static void setupFog(ActiveRenderInfo p_setupFog_0_, FogType p_setupFog_1_, float p_setupFog_2_, boolean p_setupFog_3_, float p_setupFog_4_)
    {
        ...
        else
        {
            ...
            else if (p_setupFog_3_)
            {
                ...
                // CustomWorld fog density integration
                polaroid.client.modules.impl.render.CustomWorld customWorld = 
                    polaroid.client.Polaroid.getInstance().getFunctionRegistry().getCustomWorld();
                if (customWorld != null && customWorld.isState() && customWorld.isEnabled("Кастомный туман")) {
                    float densityMultiplier = polaroid.client.modules.impl.render.CustomWorld.fogDensity.get() / 100.0F;
                    f3 = p_setupFog_2_ * 0.05F * densityMultiplier;
                    f4 = Math.min(p_setupFog_2_, 192.0F) * 0.5F * densityMultiplier;
                } else {
            ...
            else if (p_setupFog_1_ == FogType.FOG_SKY)
            {
                ...
                // CustomWorld fog density integration for sky
                polaroid.client.modules.impl.render.CustomWorld customWorld = 
                    polaroid.client.Polaroid.getInstance().getFunctionRegistry().getCustomWorld();
                if (customWorld != null && customWorld.isState() && customWorld.isEnabled("Кастомный туман")) {
                    float densityMultiplier = polaroid.client.modules.impl.render.CustomWorld.fogDensity.get() / 100.0F;
                    f3 = 0.0F;
                    f4 = p_setupFog_2_ * densityMultiplier;
                } else {
                    f3 = 0.0F;
                    f4 = p_setupFog_2_;
                }
            ...
            else
            {
                ...
                // CustomWorld fog density integration for terrain
                polaroid.client.modules.impl.render.CustomWorld customWorld = 
                    polaroid.client.Polaroid.getInstance().getFunctionRegistry().getCustomWorld();
                if (customWorld != null && customWorld.isState() && customWorld.isEnabled("Кастомный туман")) {
                    float densityMultiplier = polaroid.client.modules.impl.render.CustomWorld.fogDensity.get() / 100.0F;
                    f3 = p_setupFog_2_ * Config.getFogStart() * densityMultiplier;
                    f4 = p_setupFog_2_ * densityMultiplier;
                } else {
