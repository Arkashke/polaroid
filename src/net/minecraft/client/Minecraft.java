...
import polaroid.client.events.TickEvent;
...
import polaroid.client.modules.impl.player.ItemCooldown;
import polaroid.client.ui.mainscreen.MainScreen;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        // Всегда используем русский язык по умолчанию, если язык ещё не задан пользователем
        if (this.gameSettings.language == null || this.gameSettings.language.isEmpty()) {
            this.gameSettings.language = "ru_ru";
        }
        ...
        // ПРИНУДИТЕЛЬНО УСТАНАВЛИВАЕМ GuiScale на 2
    ...
    public void setDefaultMinecraftTitle() {
        ...
        if (selfDestruct.unhooked) {
            ...
            this.mainWindow.setWindowTitle(Polaroid.CLIENT_NAME + " - Release");
        }
    }
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        Polaroid.getInstance().getEventBus().post(tickEvent);
                ...
                for (Hand hand : Hand.values()) {
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY -> {
                                ModuleRegistry functionRegistry = Polaroid.getInstance().getFunctionRegistry();
                                Aura aura = functionRegistry.getAura();
                                ...
                                boolean auraIsStateAndTargetNotNull = aura.isState() && aura.getTarget() != null;

                                if (!auraIsStateAndTargetNotNull) {
    ...
    public void runTick() {
        ...
        if (this.world != null) {
            Polaroid.getInstance().getEventBus().post(player.tick);
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        Polaroid.getInstance().getEventBus().post(new EventChangeWorld());
    }
