...
import polaroid.client.modules.impl.misc.SelfDestruct;
    ...
    protected void init() {
        ...
        if (this.minecraft.playerController.isInCreativeMode()) {
            ...
            if (!selfDestruct.unhooked) {
                // Проверяем, находимся ли на SpookyTime сервере
                final boolean isSpookyTime;
                if (mc.getCurrentServerData() != null) {
                    String serverIP = mc.getCurrentServerData().serverIP.toLowerCase();
                    isSpookyTime = serverIP.contains("spookytime.net") || 
                                   serverIP.contains("mc.spookytime.net") ||
                                   serverIP.contains("connect.spookytime.net");
                } else {
                    isSpookyTime = false;
                }
                
                Button dropButton = new Button(width / 2 - 50, height / 2 - 125, 100, 20,
                        new StringTextComponent("Выбросить всё"), (button) -> {
                    if (mc.player != null && mc.playerController != null && !isSpookyTime) {
                        dropItems();
                    }
                });
                
                // Блокируем кнопку на SpookyTime
                dropButton.active = !isSpookyTime;
                this.addButton(dropButton);
                
                // Добавляем надпись под кнопкой если на SpookyTime (без фона и без hover эффекта)
                if (isSpookyTime) {
                    Button warningLabel = new Button(width / 2 - 75, height / 2 - 103, 150, 10,
                            new StringTextComponent("Не работает на SpookyTime!"), (button) -> {}) {
                        @Override
                        public void renderButton(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
                            // Рендерим только текст без фона
                            drawCenteredString(matrixStack, Minecraft.getInstance().fontRenderer, 
                                this.getMessage(), this.x + this.width / 2, this.y + (this.height - 8) / 2, 0xFFFFFF);
                        }
                        
                        @Override
                        protected boolean isValidClickButton(int button) {
                            return false; // Отключаем клики
                        }
                    };
                    this.addButton(warningLabel);
                }
            }
