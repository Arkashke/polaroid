...
import polaroid.client.modules.impl.misc.AuctionHelper;
import polaroid.client.modules.impl.misc.ItemScroller;
...
public abstract class ContainerScreen<T extends Container> extends Screen implements IHasContainer<T> {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i1 = 0; i1 < this.container.inventorySlots.size(); ++i1) {
            ...
            if (this.isSlotSelected(slot, (double) mouseX, (double) mouseY) && slot.isEnabled()) {
                ...
                if (Polaroid.getInstance().getFunctionRegistry().getItemScroller().isState()) {
    ...
    private void moveItems(MatrixStack matrixStack, Slot slot) {
        ...
        AuctionHelper ahHelper = moduleRegistry.getAuctionHelper();
        if(ahHelper.isState()) {
            if (ahHelper.getX() != 0) {
                int x = (int) ahHelper.getX();
                int y = (int) ahHelper.getY();
                int colorAuc1 = ahHelper.color1.get();
                int colorAuc2 = ahHelper.color2.get();
                int colorAuc3 = ahHelper.color3.get();
                fill(matrixStack, x, y, x + 16, y + 16, colorAuc1);
                if (ahHelper.getX2() != 0) {
                    int x2 = (int) ahHelper.getX2();
                    int y2 = (int) ahHelper.getY2();
                    fill(matrixStack, x2, y2, x2 + 16, y2 + 16, colorAuc2);
                }
                /* и ластовый красный */
                if (ahHelper.getX3() != 0) {
                    int x3 = (int) ahHelper.getX3();
                    int y3 = (int) ahHelper.getY3();
                    fill(matrixStack, x3, y3, x3 + 16, y3 + 16, colorAuc3);
                }
            }
        }
        this.itemRenderer.zLevel = 0.0F;
