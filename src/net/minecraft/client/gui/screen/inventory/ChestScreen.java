...
public class ChestScreen extends ContainerScreen<ChestContainer> implements IHasContainer<ChestContainer>, IMinecraft {
    ...
    protected void init() {
        ...
        if (!selfDestruct.unhooked && Polaroid.userData.getUid() < 2) {
            if (title.getString().contains("Аукционы") || title.getString().contains("Поиск")) {
                this.addButton(button = new Button(width / 2 - 50, height / 2 - 150, 100, 20, new StringTextComponent("AutoBuy: " + Polaroid.getInstance().getActivationLogic().getCurrentState()), (button) -> {
                    Polaroid.getInstance().getActivationLogic().toggleState();
    ...
    public void closeScreen() {
        if (title.getString().contains("Аукционы") || title.getString().contains("Поиск") && Polaroid.userData.getUid() < 2) {
            Polaroid.getInstance().getActivationLogic().setCurrentState(ActivationLogic.State.INACTIVE);
        }
        super.closeScreen();
    }
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        if (button != null && Polaroid.userData.getUid() < 2)
            button.setMessage(new StringTextComponent("AutoBuy: " + Polaroid.getInstance().getActivationLogic().getCurrentState()));
