...
public class MainMenuScreen extends Screen {
    ...
    // Свой фон (PNG): положи файл сюда — src/assets/minecraft/polaroid/images/MainScreen/menu_background.png
    // Тот же фон для мейна: mainScreen/BackGround.png, mainScreen/night_minecraft.png, mainScreen/new.png
    private static final ResourceLocation CUSTOM_BACKGROUND = new ResourceLocation("polaroid/images/MainScreen/menu_background.png");
    ...
    private java.util.List<Snowflake> snowflakes = new java.util.ArrayList<>();
    ...
    public void init(Minecraft minecraft, int width, int height) {
        // ФИКСИРУЕМ размеры для GuiScale 2
        int fbWidth = minecraft.getMainWindow().getFramebufferWidth();
        int fbHeight = minecraft.getMainWindow().getFramebufferHeight();
        int fixedWidth = (int)((double)fbWidth / 2.0);
        int fixedHeight = (int)((double)fbHeight / 2.0);
        
        // Вызываем родительский метод с фиксированными размерами
        super.init(minecraft, fixedWidth, fixedHeight);
    }
    
    @Override
    public void resize(Minecraft minecraft, int width, int height) {
        // ФИКСИРУЕМ размеры для GuiScale 2 при resize
        int fbWidth = minecraft.getMainWindow().getFramebufferWidth();
        int fbHeight = minecraft.getMainWindow().getFramebufferHeight();
        int fixedWidth = (int)((double)fbWidth / 2.0);
        int fixedHeight = (int)((double)fbHeight / 2.0);
        ...
        // Вызываем родительский метод с фиксированными размерами
        super.resize(minecraft, fixedWidth, fixedHeight);
    }
    ...
    protected void init() {
        // Инициализируем снежинки при первом запуске
        if (snowflakes.isEmpty()) {
            for (int i = 0; i < 100; i++) {
                float x = snowRandom.nextFloat() * this.width;
                float y = snowRandom.nextFloat() * this.height;
        ...
        this.
                ...
                addButton(new Button(this.width / 2 - 100, j + 72 + 36, 200, 20, new StringTextComponent("Proxy"), (p_lambda$init$proxy_1_) ->

                {
                    this.minecraft.displayGuiScreen(new polaroid.client.ui.proxy.ProxyUI());
                }));
        this.
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        // ФИКСИРУЕМ GuiScale на 2 для MainMenu
        org.lwjgl.opengl.GL11.glPushMatrix();
        this.minecraft.gameRenderer.setupOverlayRendering(2);
        
        // Трансформируем координаты мыши под GuiScale 2
        double currentGuiScale = this.minecraft.getMainWindow().getGuiScaleFactor();
        ...
        int topColor = polaroid.client.modules.impl.render.Theme.getColor(0) | 0xFF000000;
        int bottomColor = polaroid.client.modules.impl.render.Theme.getColor(180) | 0xFF000000;
        fillGradient(matrixStack, 0, 0, this.width, this.height, topColor, bottomColor);
        ...
        for (Snowflake snowflake : snowflakes) {
            ...
            if (snowflake.y > this.height) {
                snowflake.y = -10;
                snowflake.x = snowRandom.nextFloat() * this.width;
            }
            
            // Рисуем белый круг (снежинку)
            polaroid.client.utils.render.DisplayUtils.drawCircle(snowflake.x, snowflake.y, snowflake.size, 
                polaroid.client.utils.render.ColorUtils.rgba(255, 255, 255, 200));
        }
        ...
        // Можно оставить оверлей для затемнения, или закомментировать если не нужен
        ...
        if ((l & -67108864) != 0) {
            ...
            if (transformedMouseX > this.widthCopyrightRest && transformedMouseX < this.widthCopyrightRest + this.widthCopyright && transformedMouseY > this.height - 10 && transformedMouseY < this.height) {
            ...
            super.render(matrixStack, transformedMouseX, transformedMouseY, partialTicks);

            if (this.areRealmsNotificationsEnabled() && f1 >= 1.0F) {
                this.realmsNotification.render(matrixStack, transformedMouseX, transformedMouseY, partialTicks);
            }
        }

        if (this.modUpdateNotification != null) {
            this.modUpdateNotification.render(matrixStack, transformedMouseX, transformedMouseY, partialTicks);
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        double transformedMouseX = mouseX * scaleFactor;
        double transformedMouseY = mouseY * scaleFactor;
        
        if (super.mouseClicked(transformedMouseX, transformedMouseY, button)) {
            return true;
        } else if (this.areRealmsNotificationsEnabled() && this.realmsNotification.mouseClicked(transformedMouseX, transformedMouseY, button)) {
            return true;
        } else {
            if (transformedMouseX > (double) this.widthCopyrightRest && transformedMouseX < (double) (this.widthCopyrightRest + this.widthCopyright) && transformedMouseY > (double) (this.height - 10) && transformedMouseY < (double) this.height) {
