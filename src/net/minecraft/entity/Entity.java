...
import polaroid.client.events.EventStartRiding;
import polaroid.client.events.MovingEvent;
import polaroid.client.events.PostMoveEvent;
import polaroid.client.modules.api.ModuleRegistry;
import polaroid.client.modules.impl.combat.Hitbox;
import polaroid.client.modules.impl.render.Trails;
import polaroid.client.scripts.lua.classes.EntityClass;
...
public abstract class Entity implements INameable, ICommandSource {
    ...
    public void move(MoverType typeIn, Vector3d pos) {
        ...
        if (this.noClip) {
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                Polaroid.getInstance().getEventBus().post(move);

                pos = move.getMotion();
                ignoreHorizontal = move.isIgnoreHorizontal();
                ignoreVertical = move.isIgnoreVertical();
                
                // Обработка сброса спринта из MovingEvent (как в Assistent)
                if (!move.isSprinting() && this.isSprinting()) {
            ...
            if (this instanceof ClientPlayerEntity) {
                ...
                Polaroid.getInstance().getEventBus().post(post);
            }
    ...
    public boolean startRiding(Entity entityIn, boolean force) {
        Polaroid.getInstance().getEventBus().post(new EventStartRiding(entityIn));
