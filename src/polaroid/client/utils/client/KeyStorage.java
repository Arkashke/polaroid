...
public class KeyStorage {
    ...
    public static String getKey(int integer) {
        if (integer < 0) {
            ...
            return shortenKeyName(KeyStorage.getReverseKey(integer));
        }
    }

    private static String shortenKeyName(String keyName) {
        // Сокращаем длинные названия клавиш
        return keyName
                .replace("CAPS_LOCK", "CAPS")
                .replace("NUM_LOCK", "NUM")
                .replace("SCROLL_LOCK", "SCROLL")
                .replace("RIGHT_SHIFT", "RSHIFT")
                .replace("LEFT_SHIFT", "LSHIFT")
                .replace("RIGHT_CONTROL", "RCTRL")
                .replace("LEFT_CONTROL", "LCTRL")
                .replace("RIGHT_ALT", "RALT")
                .replace("LEFT_ALT", "LALT")
                .replace("RIGHT_SUPER", "RSUPER")
                .replace("LEFT_SUPER", "LSUPER")
                .replace("PAGEUP", "PGUP")
                .replace("PAGEDOWN", "PGDN")
                .replace("KP_DECIMAL", "NUM.")
                .replace("KP_DIVIDE", "NUM/")
                .replace("KP_MULTIPLY", "NUM*")
                .replace("KP_SUBTRACT", "NUM-")
                .replace("KP_PLUS", "NUM+")
                .replace("KP_ENTER", "NUMENTER")
                .replace("KP_EQUAL", "NUM=")
                .replace("BACKSLASH", "\\")
                .replace("APOSTROPHE", "'");
    }
