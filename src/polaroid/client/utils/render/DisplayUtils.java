...
import polaroid.client.utils.shader.ShaderUtil;
...
public class DisplayUtils implements IMinecraft {
    ...
    public static void drawRoundedRectOutline(float x, float y, float width, float height, float radius, float outlineThickness, int color) {
        GlStateManager.enableBlend();
...
// Установка цвета
        float[] col = ColorUtils.IntColor.rgb(color);
...
// Верхний левый угол
    ...
    public static void drawImageAlpha(ResourceLocation resourceLocation, float x, float y, float width, float height, Vector4i color) {
        ...
        {
            buffer.pos(x, y, 0).tex(0, 0).lightmap(0, 240).color(color.x).endVertex();
            buffer.pos(x, y + height, 0).tex(0, 1).lightmap(0, 240).color(color.y).endVertex();
            buffer.pos(x + width, y + height, 0).tex(1, 1).lightmap(0, 240).color(color.z).endVertex();
            buffer.pos(x + width, y, 0).tex(1, 0).lightmap(0, 240).color(color.w).endVertex();
    ...
    public static void drawLine3D(Vector3d start, Vector3d end, int color, float width) {
        Vector3d camPos = mc.getRenderManager().info.getProjectedView();
        
        double startX = start.getX() - camPos.getX();
        double startY = start.getY() - camPos.getY();
        double startZ = start.getZ() - camPos.getZ();
        
        double endX = end.getX() - camPos.getX();
        double endY = end.getY() - camPos.getY();
        double endZ = end.getZ() - camPos.getZ();
        ...
        bufferBuilder.pos(startX, startY, startZ)
            .color(rgba[0], rgba[1], rgba[2], rgba[3])
            .endVertex();
        bufferBuilder.pos(endX, endY, endZ)
            .color(rgba[0], rgba[1], rgba[2], rgba[3])
            .endVertex();
    ...
    public static void drawGradientRound(float x, float y, float width, float height, float radius, int color1, int color2, int color3, int color4) {
        ...
        ShaderUtil.rounded.attach();
        ShaderUtil.rounded.setUniform("size", width * 2, height * 2);
        ShaderUtil.rounded.setUniform("round", radius, radius, radius, radius);
        ShaderUtil.rounded.setUniform("smoothness", 0.0f, 1.5f);
        
        buffer.begin(GL_QUADS, POSITION_COLOR);
        buffer.pos(x, y, 0).color(color1 >> 16 & 0xFF, color1 >> 8 & 0xFF, color1 & 0xFF, color1 >>> 24).endVertex();
        buffer.pos(x, y + height, 0).color(color2 >> 16 & 0xFF, color2 >> 8 & 0xFF, color2 & 0xFF, color2 >>> 24).endVertex();
        buffer.pos(x + width, y + height, 0).color(color3 >> 16 & 0xFF, color3 >> 8 & 0xFF, color3 & 0xFF, color3 >>> 24).endVertex();
        buffer.pos(x + width, y, 0).color(color4 >> 16 & 0xFF, color4 >> 8 & 0xFF, color4 & 0xFF, color4 >>> 24).endVertex();
        tessellator.draw();
