package polaroid.client.ui.autobuy;
...
public class AutoBuyConfig implements IMinecraft {
    ...
    private static final File file = new File(mc.gameDir, "\\polaroid\\files\\autoBuy.cfg");
    ...
    public static void updateFile() {
        ...
        for (AutoBuy autoBuy : Polaroid.getInstance().getAutoBuyHandler().items) {
    ...
    private void readBuyConfig() throws FileNotFoundException {
        ...
        if (jsonObject.has("autoBuys")) {
            for (JsonElement element : jsonObject.get("autoBuys").getAsJsonArray()) {
                ...
                Polaroid.getInstance().getAutoBuyHandler().items.add(new AutoBuy(item, price,autoBuyObject.get("count").getAsInt(), enchantments,
