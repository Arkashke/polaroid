...
public class GPSCommand implements Command, CommandWithAdvice, MultiNamedCommand, IMinecraft {
    ...
    public void execute(Parameters parameters) {
        ...
        switch (commandType) {
            ...
            default -> {
                // Упрощенный синтаксис: .gps x y z или .gps x z
                try {
                    int x = Integer.parseInt(commandType);
                    int y, z;
                    
                    if (parameters.asString(1).isPresent() && parameters.asString(2).isPresent()) {
                        // .gps x y z
                        y = parameters.asInt(1).orElseThrow();
                        z = parameters.asInt(2).orElseThrow();
                    } else if (parameters.asString(1).isPresent()) {
                        // .gps x z (y = текущая высота игрока)
                        y = (int) mc.player.getPosY();
                        z = parameters.asInt(1).orElseThrow();
                    } else {
                        throw new CommandException(TextFormatting.RED + "Укажите координаты!");
                    }
                    
                    String name = "Точка_" + waysMap.size();
                    Vector3i vec = new Vector3i(x, y, z);
                    waysMap.put(name, vec);
                    logger.log("Путь " + name + " был добавлен!");
                } catch (NumberFormatException e) {
                    throw new CommandException(TextFormatting.RED + "Укажите тип команды:" + TextFormatting.GRAY + " add, remove, clear, off");
                }
            }
        }
    }
    ...
    private void onDisplay(EventDisplay e) {
        ...
        for (String name : waysMap.keySet()) {
            ...
            if (vec2f.equals(new Vector2f(Float.MAX_VALUE, Float.MAX_VALUE))) {
                ...
                // GPS точки используют обычный цвет темы (не друзья)
                Pointers.drawTriangle(false);
